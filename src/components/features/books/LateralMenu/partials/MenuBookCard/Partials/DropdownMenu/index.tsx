    ...
    >
      {statuses?.map((status, i) => (
        <div key={status.value}>
          <button
            className={`flex h-10 w-full cursor-pointer items-center gap-2.5 border-none bg-transparent px-4 text-[0.875rem] font-medium transition-colors ${
              activeStatus === status.value
                ? 'text-fg'
                : 'text-fg2 hover:bg-white/[0.05] hover:text-fg'
          ...
          {i < statuses.length - 1 && (
            <span className="mx-4 block h-px bg-white/[0.06]" />
          )}
        </div>
      ))}
      ...
      {activeStatus !== null && (
        <>
          <span className="mx-4 mt-1 block h-px bg-white/[0.06]" />
          <button
            onClick={handleRemoveFromLibrary}
            className="flex h-10 w-full items-center gap-2.5 border-none bg-transparent px-4 text-[0.875rem] font-medium text-red-400/70 transition-colors hover:bg-red-500/[0.08] hover:text-red-400"
          >
